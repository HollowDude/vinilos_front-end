...
import {
  LayoutDashboard, Palette, CircleDot,
  Package, ShoppingBag, BarChart,
...
export default function AdminSidebar({ userType, onCollapse }: Props) {
  ...
  const handleLogout = async () => {
    ...
    if (res.ok) {
      ...
      console.error("Logout fallido", await res.json());
    }
  }
  ...
  return (
    <div className={`admin-sidebar ${isCollapsed ? "admin-sidebar-collapsed" : ""}`}>      
      ...
      <nav className="admin-sidebar-nav">
        <Link href="/admin/dashboard" className={`admin-sidebar-link ${isActive("/admin/dashboard") ? "active" : ""}`}>
          <LayoutDashboard size={20}/> {!isCollapsed && "Dashboard"}
        </Link>
        {currentRole === "tatuador" && (
          <>
            <Link href="/admin/tatuajes" className={`admin-sidebar-link ${isActive("/admin/tatuajes") ? "active" : ""}`}>
              <Palette size={20}/> {!isCollapsed && "Tatuajes"}
            </Link>
            <Link href="/admin/materiales" className={`admin-sidebar-link ${isActive("/admin/materiales") ? "active" : ""}`}>
        ...
        {currentRole === "perforador" && (
          <>
            <Link href="/admin/piercings" className={`admin-sidebar-link ${isActive("/admin/piercings") ? "active" : ""}`}>
        ...
        {currentRole === "administrador" && (
          <>
            <Link href="/admin/finanzas" className={`admin-sidebar-link ${isActive("/admin/finanzas") ? "active" : ""}`}>
              <BarChart size={20}/> {!isCollapsed && "Finanzas"}
            </Link>
            <Link href="/admin/inventario" className={`admin-sidebar-link ${isActive("/admin/inventario") ? "active" : ""}`}>
              <Package size={20}/> {!isCollapsed && "Inventario"}
            </Link>
            <Link href="/admin/abastecimiento" className={`admin-sidebar-link ${isActive("/admin/abastecimiento") ? "active" : ""}`}>
              <ShoppingBag size={20}/> {!isCollapsed && "Abastecimiento"}
            </Link>
          </>
        )}
        <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="admin-sidebar-link">
