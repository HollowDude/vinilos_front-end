...
export default function VentasAdmin() {
  ...
  // initialize newProductoId when productos load
  useEffect(() => {
    if (productos.length > 0 && newProductoId === null) {
      setNewProductoId(productos[0].id)
    }
  }, [productos, newProductoId])
        ...
        <div className="ventas-table-container">
          <table className="ventas-table">
            ...
            <tbody>
              ...
              {/* formulario siempre visible al final */}
              <tr>
                <td>{new Date().toLocaleDateString('en-CA')}</td>
                <td>
                  <select
                    className="admin-form-select"
                    value={newProductoId ?? undefined}
